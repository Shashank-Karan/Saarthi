      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8 gap-4">
          <div className="flex-1">
            <h2 className="text-2xl sm:text-3xl font-serif font-bold text-foreground mb-2">Community</h2>
            <p className="text-sm sm:text-base text-muted-foreground">Share your spiritual journey and learn from others</p>
          </div>
          {user && (
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button className="shadow-lg hover:shadow-xl transition-all duration-300 w-full sm:w-auto" data-testid="button-create-post">
                  <Plus className="mr-2 h-4 w-4" />
                  <span className="hidden xs:inline">Create Post</span>
                  <span className="xs:hidden">Post</span>
                </Button>
              </DialogTrigger>
              <DialogContent className="mx-2 sm:mx-0 sm:max-w-md max-w-[96vw] w-full">
                <DialogHeader>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col xs:flex-row items-start xs:items-center gap-2">
                      ...
                      <Label
                        ...
                        className="flex items-center space-x-2 cursor-pointer bg-blue-50 hover:bg-blue-100 px-3 py-2 rounded-md border border-blue-200 transition-colors w-full xs:w-auto justify-center xs:justify-start"
                      >
                      ...
                      <Label
                        ...
                        className="flex items-center space-x-2 cursor-pointer bg-green-50 hover:bg-green-100 px-3 py-2 rounded-md border border-green-200 transition-colors w-full xs:w-auto justify-center xs:justify-start"
                      >
        ...
        {isLoading ? (
              ...
              <Card key={i} className="p-3 sm:p-4 md:p-6">
                <div className="animate-pulse">
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    <div className="h-2 sm:h-3 bg-muted rounded w-3/4"></div>
                  </div>
                </div>
              </Card>
            ))}
          ...
          <div className="text-center py-12">
            ...
            {user && (
              <Button onClick={() => setIsDialogOpen(true)} data-testid="button-first-post" className="w-full sm:w-auto mx-auto">
                <span className="hidden xs:inline">Create First Post</span>
                <span className="xs:hidden">First Post</span>
              </Button>
            )}
          </div>
        ) : (
          <div className="grid gap-3 sm:gap-4 md:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 auto-rows-max" data-testid="posts-grid">
                ...
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-center space-x-3 mb-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <h4 className="font-semibold text-foreground text-sm sm:text-base truncate">{post.author.name}</h4>
                      <p className="text-xs text-muted-foreground">
                        {safeFormatDistanceToNow(post.createdAt)}
                      </p>
                    </div>
                  </div>
                  <h3 className="font-semibold text-foreground mb-2 text-sm sm:text-base break-words" data-testid={`post-title-${post.id}`}>
                    {post.title}
                  </h3>
                  <p className="text-muted-foreground text-xs sm:text-sm mb-3 break-words" data-testid={`post-content-${post.id}`}>
                    {post.content}
                  </p>
                  ...
                  <div className="flex items-center justify-between text-xs sm:text-sm">
                    <div className="flex space-x-2 sm:space-x-4">
                      <button
                        ...
                        className="text-muted-foreground hover:text-red-500 transition-colors flex items-center space-x-1 min-w-0"
                        data-testid={`button-like-${post.id}`}
                      >
                        <Heart className="h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0" />
                        <span className="truncate">{post.likes}</span>
                      </button>
                      <button className="text-muted-foreground hover:text-primary transition-colors flex items-center space-x-1 min-w-0">
